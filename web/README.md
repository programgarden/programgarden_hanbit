...
| `/orders` | 주문/체결 내역 |
| `/charts` | 차트 (lightweight-charts 실캔들; 실시간 틱 push 는 후속) |
| `/strategies` | 자동매매 전략 — 엔진 on/off·수동 실행·결과(M5) |
| `/risk` | 위험 한도·킬스위치·**실거래 무장(arming) 토글** |
...
- 백엔드 REST/WS 실연결 완료. 국내·해외주식 LIVE 주문·자동매매 전략·실거래 무장까지 화면에서 제어.
  단 실거래는 서버 `HANBIT_ALLOW_LIVE=true`(허용) + 화면 무장(강한 확인) 둘 다 필요 — 기본은 실주문 0.
